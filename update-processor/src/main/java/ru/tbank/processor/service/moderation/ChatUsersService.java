...
public class ChatUsersService {
    ...
    public Optional<ChatUser> findChatUser(Long chatId, Long userId) {
        return Optional.ofNullable(redisChatUsersTemplate.opsForValue().get(KEY_TEMPLATE.formatted(chatId, userId)));
    }

    public void updateChatUser(ChatUser chatUser) {
        redisChatUsersTemplate.opsForValue().set(KEY_TEMPLATE.formatted(chatUser.getChatId(), chatUser.getUserId()), chatUser);
    }
}
